...
class USRNet(nn.Module):
    def __init__(self, ssf=2, n_iter=8, h_nc=64, in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
    ...
    def forward(self, x, k, sigma):
        ...
        ab = self.h(torch.cat((sigma, torch.tensor(self.sf).type_as(sigma).expand_as(sigma)), dim=1))
